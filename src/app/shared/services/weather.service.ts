...
export class WeatherService implements OnDestroy {
  ...
  getCityWeatherById(id: string): Observable<any> {
    const params = new HttpParams({fromObject: {id}});
    return this.doGet<any>('weather', params)
      .pipe(map(response => responseToCityWeather(response)));
  }

  getCityWeatherByCoord(lat: number, lon: number): Observable<any> {
  ...
  getWeatherDetails(lat: number, lon: number): Observable<any> {
    const params = new HttpParams({fromObject: {
      lat: lat.toString(),
      lon: lon.toString(),
  ...
  private doGet<T>(url: string, params: HttpParams): Observable<T> {
    ...
    params = params.append('lang', 'es_es');
